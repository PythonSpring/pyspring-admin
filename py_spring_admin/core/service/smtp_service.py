...
class SmtpService(Component):
    ...
    @classmethod
    def create_email_message(cls, receiver_email: str, subject: str, content: str, content_type: EmailContentType,  attachment_path: Optional[str] = None,  attachment_file_name: Optional[str] = None) -> EmailMessage:
        is_allowed = False
        for domain in cls.smtp_properties.allowed_domains:
            if receiver_email.endswith(domain):
                ...
                break
        if not is_allowed:
            raise EmailDomainNowAllowedError(f"Email Domain: {receiver_email} is not allowed")
    ...
    @classmethod
    def _attach_file(cls, message: EmailMessage, attachment_path: str, attachment_file_name: Optional[str] = None) -> None:
    ...
    def _send_email(self, email_message: EmailMessage) -> bool:
        try:
            with smtplib.SMTP(self.smtp_properties.host, self.smtp_properties.port) as smtp:
                smtp.starttls()
                ...
                logger.success(f"[EMAIL SENDING SUCCESSFULLY] Sending email to {email_message['To']}")
    ...
    def _handle_email_messages(self) -> None:
        def wrapper() -> None:
            while (True):
                ...
                if not is_sent:
                    logger.error(f"[EMAIL SENDING FAILED] Failed to send email to {email_message['To']}, push back to queue.")
